...
export async function notifyExpenseCreated(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    actorName: input.actorName,
    kind: "expense_new",
    title: `🔴 CHI ${fmtVndShort(input.amount)} — ${input.code} chờ KT chi`,
...
export async function notifyExpensePaid(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    actorName: input.actorName,
    kind: "expense_paid",
    title: `✅ CHI ${fmtVndShort(input.paidAmount)} — KT ${input.actorName} đã chi ${input.code}`,
...
export async function notifyExpenseKsRequest(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `🔴 KS ${fmtVndShort(input.amount)} — ${input.actorName} xin chi lẻ, chờ duyệt (${input.code})`,
...
export async function notifyExpenseTptcApproved(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `🔴 KS ${fmtVndShort(input.amount)} — TPTC đã duyệt ${input.code}, chờ KT chi`,
    body: `${input.note ?? "Mua lẻ tại công trình"}${where}`,
    link: `/expenses?id=${input.expenseId}`,
    expenseId: input.expenseId,
...
export async function notifyExpenseTptcRejected(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `❌ KS — TPTC từ chối ${input.code}`,
    body: `Lý do: ${input.reason}`,
    link: `/ks-ql/today`,
    expenseId: input.expenseId,
...
export async function notifyExpenseKtRequest(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `🔴 CHI ${fmtVndShort(input.amount)} — KT xin, chờ anh duyệt (${input.code})`,
    body: `${input.categoryName}${input.payee ? ` · ${input.payee}` : ""}${where}`,
    link: `/expenses?id=${input.expenseId}`,
    expenseId: input.expenseId,
...
export async function notifyExpenseKtApproved(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `🔴 CHI ${fmtVndShort(input.amount)} — Đã duyệt ${input.code}, chờ KT thanh toán`,
...
export async function notifyExpenseKtRejected(input: {
  ...
  await writeExpenseStaffNotif({
    ...
    title: `❌ CHI — Admin từ chối ${input.code}`,
    body: `Lý do: ${input.reason}`,
    link: `/expenses?id=${input.expenseId}`,
    expenseId: input.expenseId,
...
export async function notifyReceiptCreated(input: {
  ...
  await writeReceiptStaffNotif({
    ...
    actorName: input.actorName,
    kind: "receipt_new",
    title: `🔴 THU ${fmtVndShort(input.amount)} — ${input.code} chờ KT xác nhận thu`,
...
export async function notifyReceiptReceived(input: {
  ...
  await writeReceiptStaffNotif({
    ...
    title: `✅ THU ${fmtVndShort(input.receivedAmount)} — KT ${input.actorName} đã thu ${input.code}`,
...
export async function notifyReceiptKtRequest(input: {
  ...
  await writeReceiptStaffNotif({
    ...
    title: `🔴 THU ${fmtVndShort(input.amount)} — KT xin, chờ anh duyệt (${input.code})`,
    body: `${sourceLabel}${input.payer ? ` · ${input.payer}` : ""}${where}`,
    link: `/receipts?id=${input.receiptId}`,
    receiptId: input.receiptId,
...
export async function notifyReceiptKtApproved(input: {
  ...
  await writeReceiptStaffNotif({
    ...
    title: `🔴 THU ${fmtVndShort(input.amount)} — Đã duyệt ${input.code}, chờ KT xác nhận thu`,
...
export async function notifyReceiptKtRejected(input: {
  ...
  await writeReceiptStaffNotif({
    ...
    title: `❌ THU — Admin từ chối ${input.code}`,
    body: `Lý do: ${input.reason}`,
    link: `/receipts?id=${input.receiptId}`,
    receiptId: input.receiptId,
