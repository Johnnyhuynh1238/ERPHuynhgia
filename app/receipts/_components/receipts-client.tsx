                ...
                >
                  <div className="rt-exp" style={{ marginTop: 0, borderTop: "none", paddingTop: 0 }}>
                    <div className="col">
                      ...
                      {r.attachmentUrl && (
                        <button type="button" className="rt-link" onClick={() => setViewer({ url: `/api/receipts/${r.id}/file?type=attachment`, isPdf: r.attachmentUrl!.toLowerCase().endsWith(".pdf") })}>📎 Xem chứng từ</button>
                      )}
                    </div>
                    <div className="col">
                      {r.project && (
                      ...
                      {r.status === "received" && (
                        <>
                          ...
                          {r.receivedReceiptUrl && (
                            <button type="button" className="rt-link" onClick={() => setViewer({ url: `/api/receipts/${r.id}/file?type=received`, isPdf: r.receivedReceiptUrl!.toLowerCase().endsWith(".pdf") })}>🧾 Xem phiếu thu</button>
                          )}
                        </>
                      )}
        ...
        >
          ...
          {viewer.isPdf ? (
            <a
              ...
              className="rounded-lg bg-white px-6 py-4 text-lg font-semibold text-blue-600 shadow-2xl"
            >
              📄 Mở PDF
            </a>
          ) : (
