...
import { ExpenseCreateModal, type ExpenseCreatePrefill } from "./expense-create-modal";
...
// Form lệnh chi (CreateForm/emptyCreate + JSX + QR/OCR/upload) đã tách sang ./expense-create-modal.tsx
...
// pickPayeeName / ocrPayeeName (đọc QR + OCR tên chủ TK) đã tách sang ./expense-create-modal.tsx
...
export function ExpensesClient({
  ...
  const [createPrefill, setCreatePrefill] = useState<ExpenseCreatePrefill | undefined>(undefined);
  ...
  // Toàn bộ state/handler tạo lệnh chi (form, QR, OCR, upload, submitCreate) đã chuyển vào ExpenseCreateModal.
    ...
    <div className="lc-doc lc-scope" data-theme={theme}>
      ...
      <div className="wrap">
        ...
        <div className="head-row">
          ...
          {canCreate && (
            <button className="btn primary" onClick={() => { setCreatePrefill(undefined); setShowCreate(true); }}>
              + Lệnh chi
            </button>
          )}
        </div>
      ...
      {/* ===== POPUP: Tạo lệnh chi (component dùng chung ExpenseCreateModal) ===== */}
      {canCreate && (
        <ExpenseCreateModal
          open={showCreate}
          onClose={() => setShowCreate(false)}
          onCreated={() => { load(); loadBalance(); }}
          role={role}
