...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!(await canReport(user.id, user.role as any, task.projectId, "material"))) return NextResponse.json({ message: "Forbidden" }, { status: 403 });
    const report = await upsertMaterialReport(params.id, user.id, await req.json());
    const meta = await prisma.task.findUnique({ where: { id: params.id }, select: { code: true, name: true } });
    await logProjectActivity(prisma, {
      projectId: task.projectId,
      actorId: user.id,
      entity: "task_material_report",
      entityId: report.id,
      action: "upsert",
      summary: `Cập nhật báo cáo vật tư task ${meta?.code} "${meta?.name}"`,
      metadata: { taskId: params.id, reportDate: report.reportDate, materialName: (report as any).materialName ?? null, quantity: (report as any).quantity ?? null },
    });
