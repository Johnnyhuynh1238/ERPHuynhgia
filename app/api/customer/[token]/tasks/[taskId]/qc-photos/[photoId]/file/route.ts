...
export async function GET(_request: Request, { params }: { params: { token: string; taskId: string; photoId: string } }) {
  ...
  if (!photo) return NextResponse.json({ message: "Không tìm thấy ảnh QC" }, { status: 404 });
