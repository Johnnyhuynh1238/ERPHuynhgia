...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const existingCount = await prisma.eveningReportPhoto.count({
    where: { eveningReportId: params.id },
  });

  if (existingCount + files.length > 3) {
    return NextResponse.json({ message: "Ảnh toàn cảnh tối đa 3 ảnh" }, { status: 400 });
  }
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "reports", "evening", params.id, "site");
  ...
  for (const file of files) {
    ...
    const row = await prisma.eveningReportPhoto.create({
      data: {
        eveningReportId: params.id,
        photoUrl: `/uploads/reports/evening/${params.id}/site/${photoFilename}`,
        thumbnailUrl: `/uploads/reports/evening/${params.id}/site/${thumbFilename}`,
        caption: null,
      },
    });
  ...
  return NextResponse.json({ photos: created, message: `Đã upload ${created.length} ảnh toàn cảnh` });
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  const photo = await prisma.eveningReportPhoto.findFirst({
    where: {
      id: photoId,
      eveningReportId: params.id,
    },
  ...
  await prisma.eveningReportPhoto.delete({ where: { id: photo.id } });
  ...
  return NextResponse.json({ message: "Đã xóa ảnh toàn cảnh" });
}
