...
export async function GET(request: Request, { params }: { params: { userId: string } }) {
  ...
  if (!targetUser || targetUser.role !== UserRole.engineer) {
    return NextResponse.json({ message: "Không tìm thấy kỹ sư" }, { status: 404 });
  }
  ...
  const engineerProjectIds = await getReportProjectIdsForEngineer(targetUser.id);
