...
export async function GET(request: Request, { params }: { params: { id: string; docId: string } }) {
  const doc = await prisma.projectDocument.findFirst({
    ...
    select: { id: true, fileName: true, fileUrl: true, mimeType: true, visibleToCustomer: true },
  });
  ...
  if (token) {
    ...
    if (access.project.id !== params.id) return NextResponse.json({ message: "Không có quyền" }, { status: 403 });
    if (!doc.visibleToCustomer) return NextResponse.json({ message: "Hồ sơ chưa được chia sẻ" }, { status: 403 });
  } else {
    let current;
    try {
